...
vu = 0.1; vp = 1e4;
test = 'test4'; method = 'LR'; post = 1; index = '1'; 
...
%     Err = Biot_LR(Domn,Parm,Sol);
    eval(['Err = Biot_', method, '(Domn,Parm,Sol,post);'])
...
eRate.Properties.Description = sprintf(['======== %s ========\n ',...
    'ux = %s\n uy= %s\n p = %s\n method: %s Postprocessing: %u index: %s \n ',...
    'lmd = %.1g mu = %.1g alph = %.1g c0 = %.1g k = %.1g vu = %.1g vp = %.1g'],...
    test,ux,uy,p,method,post,index,lmd,mu,alph,c0,k,vu,vp);

disp(eRate.Properties.Description);
disp(eRate);
%% Output
file = [test, '_result']; 
name = ['eRate_',method,'_','post',num2str(post),'_',index];
eval([name,' = eRate;']);
